...
data_dict = {event['id']: event for event in data}

def stream_events(events: dict):
    for event in events.values():
        yield event


def stream_analytics(event_stream):
    count = 0
    high_level = 0
    treasures = 0
    level_up = 0
    ...
    for event in event_stream:
        ...
        if event['event_type'] == "item_found":
            ...
            level_up += 1
    return count, high_level, treasures, level_up
...
if __name__ == "__main__":
    ...
    events = stream_events(data_dict)
    first_event = next(events)
    print(f"Processing {len(data_dict)} game events...\n")
    print(f"Event 1: Player {first_event['player']} "
          f"(level {first_event['data']['level']})"
          f" {first_event['event_type']}")
    second_event = next(events)
    print(f"Event 2: Player {second_event['player']} "
          f"(level {second_event['data']['level']}) "
          f"{second_event['event_type']}")
    third_event = next(events)
    print(f"Event 3: Player {third_event['player']} "
          f"(level {third_event['data']['level']}) "
          f"{third_event['event_type']}")
    print("...\n")

    count, high_level, treasures, level_up = stream_analytics(stream_events(data_dict))
